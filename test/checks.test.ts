...
test("a check's exit code and output become assertable evidence", async () => {
  const e = await checkEnricher({ ok: { command: "echo hello; exit 0" } }, "/tmp")(turn, []);
...
test("a failing check is decided in code with no threshold", async () => {
  const e = await checkEnricher({ bad: { command: "exit 3" } }, "/tmp")(turn, []);
...
test("a check that cannot run never reads as passing", async () => {
  ...
  const e = await checkEnricher({ nope: { command: "exit 127" } }, "/definitely/not/here")(turn, []);
