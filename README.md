...
negative cases, never picked. Exact per-fixture numbers move ±0.05 between runs, so they
live in [`docs/measured.txt`](docs/measured.txt), regenerated by
`bun test/calibrate.ts --write` — not hand-typed here where they rot.
...
The `coverage` Score can't tell a real omission from a declared one — negative margin. It
stays because it's the **only** check that catches a turn with no work in it: every other
question asks "did you do something wrong", none asks "did you do anything". Remove it and
that failure mode walks through, 3 runs out of 3.
...
[measured](docs/measured.txt) · [the state problem](docs/state.txt) · [the guard](docs/guard.txt) ·
