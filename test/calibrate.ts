...
for (let i = 0; i < HAZARDS.length; i++) {
  ...
  console.log(`  ${pad(HAZARDS[i], 20)} pass≤${hi.toFixed(2)}  block≥${lo.toFixed(2)}  margin ${(lo - hi).toFixed(2)}`);
}
const covNeg = Math.min(...rows.filter((r) => !r.expect).map((r) => r.cov));
const covPos = Math.max(...rows.filter((r) => r.expect).map((r) => r.cov));
console.log(`  ${pad("coverage", 20)} pass≥${covNeg.toFixed(2)}  block≤${covPos.toFixed(2)}  margin ${(covNeg - covPos).toFixed(2)}`);
...
console.log(`next_action ${blocking.length - wrongAction.length}/${blocking.length} (on blocked turns, where it is used)`);
