...
import { compose, DEFAULTS, parseSpec } from "../orly.ts";
import { scoreSpecs, SPEC_PREFIX, specQuestions, unmet, validateSpecs, type Spec } from "../orly.ts";
import { advance, DEFAULT_MAX_ROUNDS, STALL_ROUNDS } from "../orly.ts";
...
import { evaluate, findOrlyDir, loadSpecFile, projectRoot, resolveKey } from "../orly.ts";
...
test("finds .orly from a nested working directory", () => {
  ...
  mkdirSync(j(root, ".orly", "specs"), { recursive: true });
  mkdirSync(deep, { recursive: true });
  write(j(root, ".orly", "specs", "a.spec"), "cut: 0.7\n\nis it done?");
  try {
    expect(findOrlyDir(deep)).toBe(j(root, ".orly"));
    expect(loadSpecFile(deep)?.specs).toHaveLength(1);
  } finally {
...
test("a spec file that does not parse blocks instead of dropping every check", () => {
  const root = mkdtempSync(j(tmpdir(), "orly-badjson-"));
  mkdirSync(j(root, ".orly", "specs"), { recursive: true });
  try {
    for (const text of ["mystery: x\n\nsome body", "cut: twelve\n\nstill bad"]) {
      write(j(root, ".orly", "specs", "broken.spec"), text);
      const [spec] = loadSpecFile(root)!.specs;
      expect(scoreSpecs([spec], {}, 0.5)[0].met).toBe(false);
      expect(validateSpecs([spec])[0].problem).toContain("malformed");
    }
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
...
test("check results never reach the model, only the require specs", async () => {
  ...
  try {
    const { judge } = await import("../orly.ts");
    await judge(
...
test("a hung keyCommand times out with its children, leaving no key and no orphan", async () => {
  const root = mkdtempSync(join(tmpdir(), "orly-key-"));
  const saved = { key: process.env.TYPESAFE_API_KEY, ms: process.env.ORLY_KEY_TIMEOUT_MS };
  try {
    mkdirSync(join(root, ".orly"));
    delete process.env.TYPESAFE_API_KEY;
    ...
    if (saved.ms === undefined) delete process.env.ORLY_KEY_TIMEOUT_MS;
    else process.env.ORLY_KEY_TIMEOUT_MS = saved.ms;
    rmSync(root, { recursive: true, force: true });
  }
});
...
test("a malformed spec file's block names what is wrong with the file", () => {
  const v = compose({}, DEFAULTS, [parseSpec("bad", "cut: 2\n\nDoes `command_results` show tests passing?\n")]);
  expect(v.block).toBe(true);
  expect(v.reason).toContain('- spec "bad" cannot be judged: malformed spec file: cut must be a number between 0 and 1, got "2"');
});
